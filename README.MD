...
系统设置中可以开启“每日流量报告”，默认在 `23:59` 按控制台时区生成报告，并通过已启用的通知通道发送。报告包含每个实例的今日增量、累计流量、额度、使用率和实例状态。

启用 Telegram 通知并保存 Bot Token 与 Chat ID 后，可以在对应 Chat 中发送以下命令交互获取实时报告：
...
Bot 只响应配置中的 Chat ID，其他会话的消息会被忽略。
