...
function GeneralSettings({ config, onChange }: { config: Config; onChange: (config: Config) => void }) {
  ...
  return <div className="settings-section"><SectionTitle icon={<Gauge />} title="监控策略" subtitle="AUTOMATION POLICY" />
    <div className="form-grid settings-form">
      ...
      <Field label="日报时间"><input type="time" value={config.daily_report_time} onChange={(event) => onChange({ ...config, daily_report_time: event.target.value })} /></Field>
    ...
    <div className="toggle-list">
      ...
      <ToggleRow title="每日流量报告" icon={<HistoryIcon />} checked={config.enable_daily_report} onChange={(checked) => onChange({ ...config, enable_daily_report: checked })} />
...
function AboutSettings({ notify }: { notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  return <div className="settings-section about-section"><SectionTitle icon={<Info />} title="关于 CDT Monitor" subtitle="PROJECT INFORMATION" /><div className="about-version"><div><span>当前版本</span><b>{info?.version || '加载中...'}</b><small>{info?.commit && info.commit !== 'unknown' ? `${info.commit} · ${info.built_at}` : '构建信息未知'}</small></div><button className="button button--secondary button--small" onClick={() => void checkVersion()} disabled={checking}>{checking ? <LoaderCircle className="spin" /> : <RefreshCw />}检查更新</button></div>{info?.latest_version && <p className="inline-hint">GitHub 最新版本：{info.latest_version}{info.latest_version === info.version ? '，当前已是最新版本' : '，请查看发布页获取更新'}</p>}<div className="about-links"><a href="https://github.com/JudiLite/CDT-Monitor" target="_blank" rel="noreferrer"><SiteFavicon domain="github.com" label="GitHub" /><span><b>GitHub 仓库</b><small>源代码、Issue 与 Release</small></span><ExternalLink /></a></div></div>
}
